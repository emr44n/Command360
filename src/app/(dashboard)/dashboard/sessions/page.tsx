...
export default async function SessionsPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <p className="text-[10px] ff-mono uppercase tracking-[0.1em] text-muted-foreground/60 font-medium mb-1.5">Dashboard</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        <div className="relative bg-card border border-border rounded-none p-5 hover:border-border/80 transition-all duration-300 overflow-hidden group">
        ...
        <div className="relative bg-card border border-border rounded-none p-5 hover:border-border/80 transition-all duration-300 overflow-hidden group">
          ...
          <div className="flex items-center gap-2">
            ...
            {activeCount > 0 && (
              <span className="relative flex h-2 w-2">
                <span className="absolute -inset-1 rounded-full bg-[#2E9E63]/20 blur-sm" />
          ...
          <p className={`text-2xl font-bold mt-1.5 ${activeCount > 0 ? 'text-[#2E9E63]' : 'text-foreground'}`}>{activeCount}</p>
        </div>
        <div className="relative bg-card border border-border rounded-none p-5 hover:border-border/80 transition-all duration-300 overflow-hidden group">
        ...
        <div className="relative bg-card border border-border rounded-none p-5 hover:border-border/80 transition-all duration-300 overflow-hidden group">
          ...
          <p className="text-[10px] ff-mono uppercase tracking-[0.1em] text-muted-foreground/60 font-medium">Responses</p>
          <p className="text-2xl font-bold text-foreground mt-1.5">{totalResponses}</p>
        </div>
      </div>

      <p className="text-[10px] ff-mono uppercase tracking-[0.1em] text-muted-foreground/60 font-medium mb-2.5">All Activity</p>
