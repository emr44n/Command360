...
import { AuthAwareCTA } from '@/components/solutions/AuthAwareCTA'
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return { title: 'Solution — Command 360' }
  return {
    title: `${service.name} — Command 360`,
          ...
          <div className="max-w-3xl mx-auto relative bg-primary rounded-3xl px-8 py-14 text-center overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                <Link
                  href="/register"
                  className="inline-flex items-center gap-2 px-8 h-12 rounded-full text-base font-medium bg-white text-primary btn-animated"
                >
                  Create a session <ArrowRight className="w-4 h-4" />
                </Link>
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 px-8 h-12 rounded-full text-base font-medium border border-white/30 text-primary-foreground hover:bg-white/10 transition-colors"
                >
                  Book a demo
                </Link>
              </div>
