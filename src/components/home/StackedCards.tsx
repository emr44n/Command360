...
/**
 * Phase-lifecycle cards.
 *
 * Desktop (sm+): a pinned scroll-stacking deck — a tall section pins a 100vh
...
/** The card surface itself (shared by mobile + desktop). */
function PhaseCardInner({ item }: { item: StackItem }) {
  return (
...
/** Desktop deck card — absolutely positioned, flings up via scroll. */
function PhaseCard({ item, z, y, slot }: { item: StackItem; z: number; y?: MotionValue<string>; slot: string }) {
  return (
    <motion.div className={`absolute left-0 right-0 ${slot}`} style={{ zIndex: z, y }}>
      <PhaseCardInner item={item} />
    </motion.div>
  )
}

function PinnedStack({ items }: { items: StackItem[] }) {
  ...
  const SLOT1 = 'top-0 bottom-[80px]'
  const SLOT2 = 'top-[40px] bottom-[40px]'
  const SLOT3 = 'top-[80px] bottom-0'

  return (
    <section ref={ref} className="relative h-[280vh]">
      <div className="sticky top-0 h-screen overflow-hidden flex items-center justify-center px-6 py-[clamp(72px,12vh,128px)]">
        <div className="relative w-full max-w-[1080px] h-[clamp(500px,76vh,648px)]">
...
export function StackedCards({ items }: { items: StackItem[] }) {
  return (
    <>
      ...
      {/* Desktop: pinned scroll-stacking deck */}
      <div className="hidden sm:block">
        <PinnedStack items={items} />
      </div>
    </>
  )
}
