...
function LayoutPreview({ layout }: { layout: CctvLayout }) {
  const cellClass = 'bg-[#3f4147] dash-light:bg-[#EAE6DD] rounded-none'
  switch (layout) {
    case '1':
      return (
...
export function StudioCctvEditor({ content, onContentChange, slides, currentSlideId }: StudioCctvEditorProps) {
  ...
  return (
    <div className="flex flex-col min-w-0 bg-[#2b2d31] dash-light:bg-white text-white dash-light:text-[#16191E]">
      {/* Header */}
      <div className="px-2 py-1.5 border-b border-[#1e1f22] dash-light:border-black/10 flex items-center gap-1.5">
        <Monitor className="w-3.5 h-3.5 text-[#6a5ea8] dash-light:text-[#5B4F98]" />
        <span className="text-[10px] font-semibold text-[#9aa0a8] dash-light:text-[#5B6169]">CCTV Configuration</span>
      </div>

      <div className="overflow-y-auto p-2 space-y-3">
        {/* Layout selector — compact */}
        <div>
          <label className="text-[9px] font-medium text-[#9aa0a8] dash-light:text-[#5B6169] mb-1.5 block">
            Layout
          </label>
                ...
                <button
                  ...
                  className={`flex items-center justify-center w-8 h-8 rounded-none border transition-all cursor-pointer ${
                    isActive
                      ...
                      : 'border-[#3f4147] dash-light:border-black/10 bg-[#1e1f22] dash-light:bg-[#ECE9E1] text-[#9aa0a8] dash-light:text-[#5B6169] hover:border-[#9aa0a8] hover:text-white dash-light:hover:text-[#16191E]'
                  }`}
                >
                  <div className="w-5">
...
function SceneSlotDropdown({
  ...
  return (
    <div ref={dropdownRef} className="relative flex items-center gap-1.5">
      <span className="text-[8px] font-bold text-[#9aa0a8] dash-light:text-[#5B6169] w-3 shrink-0">{index + 1}</span>
      {/* Thumbnail of assigned scene */}
      <div className="w-10 min-w-[2.5rem] max-w-[5rem] flex-[0_0_auto] aspect-video rounded-none bg-[#1e1f22] dash-light:bg-[#ECE9E1] overflow-hidden border border-[#3f4147] dash-light:border-black/10 relative" style={{ width: 'clamp(2.5rem, 20%, 5rem)' }}>
        {assignedContent ? (
          ...
          <div className="w-full h-full bg-[#1e1f22] dash-light:bg-[#ECE9E1]" />
        )}
      </div>
      {/* Dropdown trigger */}
      <button
        ...
        className="flex-1 h-6 flex items-center justify-between text-[9px] bg-[#1e1f22] dash-light:bg-[#ECE9E1] border border-[#3f4147] dash-light:border-black/10 rounded-none text-[#9aa0a8] dash-light:text-[#5B6169] px-1.5 cursor-pointer hover:border-red-500/50 transition-colors"
      >
        <span className="truncate">{assignedLabel}</span>
        <ChevronDown className={`w-2.5 h-2.5 shrink-0 text-[#9aa0a8] dash-light:text-[#5B6169] transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {/* Dropdown menu — fixed, grid of scene thumbnails */}
      {open && (
        <div ref={dropdownRef} className="fixed z-[9999] bg-[#1e1f22] dash-light:bg-[#F5F2EB] border border-[#3f4147] dash-light:border-black/10 rounded-none shadow-2xl p-1.5 max-h-72 overflow-y-auto"
          ...
          <div
            ...
            className="flex items-center gap-1.5 px-1.5 py-1 hover:bg-[#35363c] dash-light:hover:bg-black/[0.05] cursor-pointer rounded-none transition-colors mb-0.5"
          >
            <div className="w-6 h-4 rounded-none bg-[#2b2d31] dash-light:bg-white border border-dashed border-[#3f4147] dash-light:border-black/10 shrink-0" />
            <span className="text-[8px] text-[#9aa0a8] dash-light:text-[#5B6169] italic truncate">Empty</span>
          </div>
          <div className="h-px bg-[#2b2d31] dash-light:bg-black/10 my-0.5" />
          ...
          {availableSlides.map((s) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-1.5 px-1.5 py-1 cursor-pointer rounded-none transition-colors mb-0.5 ${isSelected ? 'bg-red-500/20 ring-1 ring-red-500/40' : 'hover:bg-[#35363c] dash-light:hover:bg-black/[0.05]'}`}
              >
                <div className="w-6 h-4 rounded-none bg-[#1e1f22] dash-light:bg-[#ECE9E1] overflow-hidden shrink-0 border border-[#3f4147] dash-light:border-black/10 relative">
                ...
                <span className={`text-[8px] truncate flex-1 ${isSelected ? 'text-red-300 font-medium' : 'text-[#9aa0a8] dash-light:text-[#5B6169]'}`}>{sceneName}</span>
      ...
      <div
        ...
        className={`rounded-none border flex flex-col items-center justify-center gap-2 p-3 transition-all ${
          isAssigned
            ...
            : 'border-[#3f4147] dash-light:border-black/10 bg-[#2b2d31] dash-light:bg-white'
        }`}
      >
        <span className="text-[10px] font-bold text-[#9aa0a8] dash-light:text-[#5B6169] uppercase">Slot {i + 1}</span>
        {isAssigned ? (
          <span className="text-xs font-medium text-white dash-light:text-[#16191E] truncate max-w-full px-1">
            {getSlideLabel(assignedId)}
          </span>
        ) : (
          <span className="text-[11px] text-[#9aa0a8] dash-light:text-[#5B6169] italic">Empty</span>
        )}
        <select
          value={assignedId}
          onChange={(e) => onSlotChange(i, e.target.value)}
          className="w-full h-6 text-[10px] bg-[#1e1f22] dash-light:bg-[#ECE9E1] border border-[#3f4147] dash-light:border-black/10 rounded-none px-1 text-[#9aa0a8] dash-light:text-[#5B6169] outline-none focus:border-[#6a5ea8] cursor-pointer"
        >
