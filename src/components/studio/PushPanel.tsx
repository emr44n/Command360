...
const TYPE_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  image: { bg: 'bg-[#3E6DC4]/20', text: 'text-[#3E6DC4]', label: 'IMG' },
  video: { bg: 'bg-[#6a5ea8]/20', text: 'text-[#6a5ea8]', label: 'VID' },
  text: { bg: 'bg-white/10', text: 'text-white', label: 'TXT' },
  shape: { bg: 'bg-[#9aa0a8]/20', text: 'text-[#9aa0a8]', label: 'SHP' },
  audio: { bg: 'bg-[#2E9E63]/20', text: 'text-[#2E9E63]', label: 'AUD' },
}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 border-b border-[#2b2d31] shrink-0">
        <h3 className="text-[9px] font-bold text-[#9aa0a8] uppercase tracking-wider">Push to Live</h3>
        {queue.length > 0 && <p className="text-[8px] text-[#9aa0a8] mt-0.5">{queue.length} item{queue.length !== 1 ? 's' : ''} ready</p>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {queue.length === 0 ? (
          <div className="text-center py-8">
            <Zap className="w-5 h-5 text-[#9aa0a8] mx-auto mb-2" />
            <p className="text-[9px] text-[#9aa0a8]">Drop assets on the canvas</p>
            <p className="text-[8px] text-[#9aa0a8] mt-1">They&apos;ll appear here ready to push</p>
          </div>
        ) : (
          <div className="space-y-1">
            {queue.map(item => {
              ...
              return (
                <div key={item.layer.id} className="bg-[#2b2d31] rounded-none overflow-hidden">
                  {/* Main row */}
                  <div className="flex items-center gap-1.5 px-2 py-1.5">
                    ...
                    {/* Name */}
                    <span className="flex-1 text-[9px] text-[#9aa0a8] truncate">{item.layer.name}</span>
                    {/* Expand for options */}
                    <Tooltip><TooltipTrigger asChild>
                    <button onClick={() => setExpandedId(isExpanded ? null : item.layer.id)} className="p-0.5 text-[#9aa0a8] hover:text-white transition-colors">
                    ...
                    {/* Remove */}
                    <Tooltip><TooltipTrigger asChild>
                    <button onClick={() => onRemoveItem(item.layer.id)} className="p-0.5 text-[#9aa0a8] hover:text-red-400 transition-colors">
                  ...
                  {isExpanded && (
                    <div className="px-2 pb-2 pt-0.5 border-t border-[#3f4147]">
                      <p className="text-[7px] text-[#9aa0a8] uppercase tracking-wider mb-1">Transition</p>
                      <div className="flex gap-1">
                        <button
                          onClick={() => onUpdateTransition(item.layer.id, 'fade')}
                          className={`flex-1 text-[8px] py-1 rounded-none transition-colors ${item.transition === 'fade' ? 'bg-red-500/20 text-red-400' : 'bg-[#1e1f22] text-[#9aa0a8] hover:text-white'}`}
                        >
                          Fade In
                        </button>
                        <button
                          onClick={() => onUpdateTransition(item.layer.id, 'instant')}
                          className={`flex-1 text-[8px] py-1 rounded-none transition-colors ${item.transition === 'instant' ? 'bg-red-500/20 text-red-400' : 'bg-[#1e1f22] text-[#9aa0a8] hover:text-white'}`}
                        >
                          Instant
                        </button>
                      </div>
