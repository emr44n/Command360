  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#1e1f22] border border-[#3f4147] rounded-none p-6 max-w-sm w-full shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#9aa0a8]">{sceneName}</p>
            </div>
          </div>
          <button onClick={onCancel} className="p-1 text-[#9aa0a8] hover:text-white rounded-none hover:bg-white/10 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>

        <p className="text-[11px] text-[#9aa0a8] mb-4 leading-relaxed">
        ...
        <div className="mb-4">
          <label className="text-[10px] text-[#9aa0a8] font-medium mb-1.5 block">Presenter Name (optional)</label>
          <input
            value={name}
            onChange={e => setName(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') onStart(name) }}
            placeholder="Enter your name..."
            className="w-full h-8 px-3 text-[11px] bg-[#2b2d31] border border-[#3f4147] rounded-none text-white placeholder:text-[#9aa0a8] outline-none focus:border-red-500/50 transition-colors"
          />
        </div>

        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 py-2 text-[11px] font-medium rounded-none bg-[#2b2d31] text-[#9aa0a8] hover:bg-[#35363c] transition-colors">
            Cancel
          </button>
